...
- **No cap on repeated redelivery of a "poison pill"**: if a specific input reliably crashes whatever worker picks it up, Celery's Redis transport will keep redelivering it every `visibility_timeout` seconds indefinitely - `CELERY_TASK_MAX_RETRIES` only governs *caught, in-process* retries, not broker-level redelivery after a lost worker. Reconciliation does not close this one: `mark_running()` refreshes `started_at` on every redelivered re-execution attempt, so a row that's still genuinely (if repeatedly) cycling never looks stale to a `started_at`-based threshold - this is correct behavior for reconciliation's actual job (distinguishing abandoned from active work), not a fix for the poison-pill problem specifically.
...
It is a plain function operating purely on Postgres, not a new Celery beat schedule - invoked via `python -m voxmind.workers.reconciliation` (a thin CLI entrypoint), intended to be run periodically by whatever an operator already has available (host cron, a Docker/Kubernetes scheduled job, or manually after a known incident). See [DECISIONS/0017](DECISIONS/0017-pipeline-run-reconciliation.md) for why a new always-on scheduler process wasn't added as part of this. It never touches `pipeline_runs` over HTTP - same guard (`tests/unit/test_pipeline_run_not_exposed.py`) as everything else in this table.
