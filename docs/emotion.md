...
- **License, clarified (final hardening pass)**: the FunASR *framework* (the code doing the loading) is MIT-licensed, confirmed directly from its repository. The `emotion2vec_plus_base` model card itself lists a non-standard `license: other, license_name: model-license`, not a standalone license file of its own. Investigated further this pass: FunASR's own repository explicitly states its pretrained model weights are licensed *separately* from its (MIT) toolkit code, "when a model card links to the FunASR Model Open Source License Agreement, those terms apply" - and that named agreement (Version 1.1, Alibaba Group, verified directly against [modelscope/FunASR's MODEL_LICENSE file](https://github.com/modelscope/FunASR/blob/main/MODEL_LICENSE)) is genuinely permissive: use/copy/modify/share are all explicitly permitted, commercial use is not prohibited, and the only substantive requirement is attribution (retaining source/author/model-name information). This is very likely what emotion2vec+'s `model-license` tag refers to, matching FunASR's own stated pattern for its models, but this project did **not** find an explicit, direct hyperlink from emotion2vec+'s own model card to that specific document - so this is reported as a strong, well-supported inference from FunASR's own documented licensing pattern, not a fact directly confirmed on the model card itself. Meaningfully more resolved than "indirect/ambiguous" (the prior characterization), but still short of a first-party, unambiguous statement.
...
- The emotion2vec+ model card's own license tag is indirect/non-standard, but is very likely FunASR's own permissive Model Open Source License Agreement (attribution required, commercial use not prohibited) based on FunASR's own documented licensing pattern - not directly confirmed via an explicit link on the model card itself, so still reported as a strong inference rather than a fully first-party-confirmed fact. See "Datasets"/licensing notes above.
