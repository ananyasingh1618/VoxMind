...
- **EMO-DB**: the original host (`emodb.bilderbar.info`) is unreachable (verified - the server actively resets connections); obtained via the `renumics/emodb` HuggingFace mirror instead. **License not independently confirmed** - the mirror has no license field set and the original terms page is unreachable to verify directly; see `docs/emotion.md`'s "Known, honest limitations" for the full statement. Parsed by `emodb.py`, which also recovers real speaker ids from the mirror's `(age, gender)` columns against the dataset's own published speaker demographics table (verified empirically against the real downloaded data - see the module's docstring).
